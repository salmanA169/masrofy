...
fun MainScreen(
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        TopBarDetails(currentMonth = mainScreenState.currentDate.month.name, onNextMonth = rememberOnNext, onPreviousMonth = rememberOnPre)
...
fun TransactionGroupList(
    ...
    LazyColumn(modifier = Modifier
        ...
        transactionGroup.forEach { transaction ->
            ...
            itemShapes(transaction.toSS(), key = {
                it.id
            }) { item, shape, shouldShowDivider ->
                val rememberShape = remember{
                    shape
                }
                com.masrofy.screens.mainScreen.TransactionItems(
                    transaction = item,
                    navController = navController,
                    rememberShape,
                    modifier = Modifier
...
@Immutable
data class SS(val id:Int,val category :Int,val categoryString:String, val comment:String?,val amount:Long,val type:TransactionType)
...
//        shape = shape
        ...
        Row(
            ...
            Column(modifier = Modifier.align(CenterVertically)) {
                Text(text = transaction.categoryString, fontSize = 14.sp)
...
fun TransactionGroup.toSS(): List<com.masrofy.screens.mainScreen.SS>{
    return transactions.map {
        com.masrofy.screens.mainScreen.SS(
            it.transactionId,
            it.category.icon,
            it.category.name,
            it.comment,
